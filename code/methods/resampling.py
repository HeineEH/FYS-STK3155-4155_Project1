...
class BiasVariance:
     def __init__(self,x,y,max_degree = 15,test_size = 0.2,n_boostraps = 100): 
          self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(x.reshape(-1,1), y.reshape(-1,1), test_size=test_size)
     ...
     def bootstrap(self):
        ...
        for degree in range(1,self.max_degree+1):
            ...
            for i in range(self.n_boostraps):
                x_, y_ = resample(self.x_train, self.y_train,random_state = rng)
